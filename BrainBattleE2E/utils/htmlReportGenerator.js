...
function generateHtmlReport(results, summaryData) {
  const OUTPUT_DIR = path.join(__dirname, '..', 'Test_Results', 'HTML');
  const REPORT_PATH = path.join(OUTPUT_DIR, 'execution-report.html');
  ...
  let failedTests = results.filter(r => r.status === 'failed').length;
...
<head>
  <meta charset="UTF-8">
  <title>BrainBattle E2E Execution Report</title>
  <style>
...
<body>
  <h1>BrainBattle E2E Test Report</h1>
  <div class="summary-card">
    ...
    <div class="stat">
      <h3 class="passed-text">${passedTests}</h3>
      <p>Passed</p>
    </div>
    <div class="stat">
      <h3 class="failed-text">${failedTests}</h3>
      <p>Failed</p>
    </div>
  </div>
  ...
  console.log(`HTML report generated at: ${REPORT_PATH}`);
}
